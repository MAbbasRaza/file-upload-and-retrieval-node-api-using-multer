...
const fileFilter = (req, file, cb) => {
  ...
  if (
    ...
    cb(
      new Error(
        "Invalid file type. Only JPEG, PNG, GIF, WEBP, MP4, MPEG, MOV, AVI, PDF, and APK files are allowed."
      ),
      false
    );
  }
...
const validateFileSize = (req, res, next) => {
  upload(req, res, (err) => {
    ...
    // Check size limits
    if (
      (isImage && fileSize > maxImageSize) ||
      (!isImage && fileSize > maxOtherSize)
    ) {
      ...
      return res.status(400).json({
        ...
        message: `File size (${formatFileSize(
          fileSize
        )}) exceeds the limit (${formatFileSize(limit)})`,
        actualSize: fileSize,
